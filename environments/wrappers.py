...
class FrameStackWrapper(dm_env.Environment):
    ...
    def __init__(self, env, num_frames, frame_keys=None, add_temporal_dim=False):
        self._env = env
        self._num_frames = num_frames
        self.add_temporal_dim = add_temporal_dim
        ...
        for key in frame_keys:
            ...
            if self.add_temporal_dim:
                shape = np.array([num_frames, frame_shape[2], frame_shape[0], frame_shape[1]])
            else:
                shape = np.concatenate([[frame_shape[2] * num_frames], frame_shape[:2]], axis=0)
            wrapped_obs_spec[key] = specs.BoundedArray(shape=shape,
                                                       dtype=frame_dtype,
                                                       minimum=0,
                                                       maximum=255,
                                                       name="observation")
    ...
    def _transform_observation(self, time_step):
        ...
        for i, key in enumerate(self._frame_keys):
            ...
            if self.add_temporal_dim:
                stacked_frames = np.stack(list(self._frames[i]), axis=0)
            else:
    ...
    def reset(self):
        ...
        for i, key in enumerate(self._frame_keys):
            # Fix bug where after reset the first rendered frame is not updated
            # pixels = self._extract_pixels(time_step, key)
            pixels = self._env.physics.render(**self._render_kwargs).transpose(2, 0, 1).copy()
