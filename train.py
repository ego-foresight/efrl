...
import utils
# from environments.distracting_dmc import make_distracting_dmc
from environments.metaworld_dm_env import make_metaworld
# from environments.hurdle_dmc import make_hurdle_dmc
from logger import Logger
from replay_buffer import ReplayBufferStorage, make_replay_loader
from video import FrameRecorder, TrainVideoRecorder
...
def make_env(cfg, is_eval):
    if cfg.task_name.split("_", 1)[0] == "metaworld":
        env = make_metaworld(
            ...
            cfg.slim_mask_cfg, cfg.agent.add_temporal_dim)
    ...
    #         cfg.slim_mask_cfg, cfg.agent.add_temporal_dim)
    # elif 'domain_name' in cfg and (cfg.domain_name.split("_", 1)[0] == "hurdle" or cfg.domain_name.split("_", 1)[0] == "ant"):
    #     env = make_hurdle_dmc(cfg.domain_name, cfg.task_name, cfg.seed, cfg.obs_type,
    #                           cfg.action_repeat, cfg.img_size, add_segmentation_to_obs=cfg.add_segmentation_to_obs)
    #                           # image_to_info, full_state_to_info)
    return env
...
class Workspace:
    ...
    def __init__(self, cfg, work_dir):
        ...
        self.replay_loader = make_replay_loader(
            ...
            self.cfg.has_success_metric, self.cfg.agent.context_steps, self.cfg.agent.horizon,
            cfg.agent.requires_extended_obs)
        ...
        self.video_recorder = FrameRecorder(
            self.work_dir if self.cfg.save_video else None, output_size=(256, 256),
            use_wandb=self.cfg.use_wandb)
    ...
    def eval(self):
        ...
        while eval_until_episode(episode):
            ...
            with torch.no_grad(), utils.eval_mode(self.agent):
                action = torch.zeros(self.eval_env.action_spec().shape, dtype=torch.float32)
            ...
            while not time_step.last():
                ...
                with torch.no_grad(), utils.eval_mode(self.agent):
                    ...
                    cur_rec_loss = self.video_recorder.record(self.agent, time_step.observation, action)
                    rec_loss =  cur_rec_loss if cur_rec_loss is not None else rec_loss
                if self.cfg.has_success_metric:
                    total_reward += time_step.reward["reward"]
                    success = int(time_step.reward["success"])
                    current_episode_max_success = max(current_episode_max_success, success)
        ...
        with self.logger.log_and_dump_ctx(self.global_frame, ty="eval") as log:
            ...
            if self.cfg.has_success_metric:
                ...
                log("mean_rec_loss", mean_rec_loss / episode)
    ...
    def train(self):
        ...
        self.train_video_recorder.init(time_step.observation['pixels'][-1])
        metrics = None
        while train_until_step(self.global_step):
            ...
            self.replay_storage.add(time_step)
            self.train_video_recorder.record(time_step.observation['pixels'][-1])
            episode_step += 1
            self._global_step += 1
